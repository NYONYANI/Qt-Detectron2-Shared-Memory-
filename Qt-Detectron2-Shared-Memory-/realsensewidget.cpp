...
void RealSenseWidget::updateFrame()
{
    try {
        ...
        if (!m_detectionResults.isEmpty()) {
            // ✨ [수정] 3D 포인트 클라우드용 오버레이에는 텍스트를 그리지 않도록 false 전달
        ...
        if (!m_detectionResults.isEmpty()) {
            // ✨ [수정] 2D 라벨용 이미지에는 텍스트를 그리도록 기본값(true) 사용 (호출 변경 없음)
...
QImage RealSenseWidget::cvMatToQImage(const cv::Mat &mat)
{
    ...
    // ✨ [수정] QImage::Format_BGR888 -> QImage::Format_RGB888
    // BGR(cv::Mat) 데이터를 RGB888 포맷으로 강제로 읽게 하여 QImage를 생성 (이때 R과 B가 바뀜)
    // 이후 .rgbSwapped()를 호출하여 R과 B를 다시 맞바꿔 올바른 RGB QImage를 반환
...
void RealSenseWidget::drawMaskOverlay(QImage &image, const QJsonArray &results, bool drawTextLabels) {
    ...
    for (const QJsonValue &value : results) {
        ...
        for (const QString &part : parts) {
            ...
            // ✨ [수정] 텍스트 그리기 로직을 if문으로 감쌈
            if (drawTextLabels) {
                painter.setPen(Qt::white); painter.setFont(QFont("Arial", 12, QFont::Bold));
