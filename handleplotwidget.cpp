...
void HandlePlotWidget::paintEvent(QPaintEvent *event)
{
    ...
    // 3. 피팅된 2차 곡선 (비활성화)
    // drawPolynomialCurve(painter, QColor(255, 0, 0, 200), 1.5);
...
void HandlePlotWidget::calculateCenterlinePoints()
{
    ...
    for (int i = 0; i <= numSamples; ++i)
    {
        ...
        if (!xValues.isEmpty())
        {
            ...
            data.midPoint = QPointF(medianX, medianY);
            data.valid = true;
        }
        else
        {
...
void HandlePlotWidget::smoothCenterlinePoints(int windowSize) // 홀수 사용 권장
{
    ...
    for (int i = firstValidIdx; i <= lastValidIdx; ++i)
    {
        if (isValidSmoothed[i]) {
            ...
            QPointF p2 = tempSmoothed[nextValidIdx];

            int gapLen = nextValidIdx - prevValidIdx;
