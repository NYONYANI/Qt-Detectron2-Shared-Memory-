...
void PointCloudWidget::drawPole()
{
    ...
    if(quadric) {
        ...
        glPushMatrix();
        // 봉의 시작점 (X, Y_START, Z)로 이동
        glTranslatef(pole_x, pole_y_start, pole_z);

        // gluCylinder는 Z축을 따라 그려지므로, Z축을 +Y축으로 회전시킴
        // (X축 기준 -90도 회전)
        ...
        // 판의 4개 꼭짓점 계산 (Y좌표는 pole_y_start로 고정)
        QVector3D p1(pole_x - plate_half_width, pole_y_start, new_top_z); // Top-Left
        QVector3D p2(pole_x + plate_half_width, pole_y_start, new_top_z); // Top-Right
        QVector3D p3(pole_x + plate_half_width, pole_y_start, new_bottom_z); // Bottom-Right
        QVector3D p4(pole_x - plate_half_width, pole_y_start, new_bottom_z); // Bottom-Left
